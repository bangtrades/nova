...
| ~~NovaCompanion scheme does not build~~ → **resolved Jun 11** | pre-existing (discovered Jun 10) | 9 defects fixed (cross-target palette refs, phantom `handleAppleSignIn` API, invalid SwiftUI initializers, `if let`-shadow mutations, wrong init arg order) — the scheme compiles for the first time. Compiles ≠ tested: a Companion smoke run is the follow-up when the parent surface matters. See `sprint-runs/companion-build-fix-2026-06-11.md`. |
| ~~iOS `/auth/signin` ↔ backend `/auth/apple` mismatch~~ → **resolved Jun 10** | pre-existing | Five breaks fixed across the seam (path, body, key-case, response shapes, refresh rotation); backend now tolerant-reader on auth bodies; wire bytes pinned by `tests/authContract.test.ts`. See `sprint-runs/auth-contract-fix-2026-06-10.md`. |
| ~~Encode-side contract audit~~ → **resolved Jun 10** | discovered Jun 10 | Encoder switched to `.useDefaultKeys` (backend is uniformly camelCase); every body audited; both live kid-path seams (`/dashy/chat` — incl. a dead-host placeholder router that was never swapped — and `/progress/sync`) were fully broken and are fixed; 3 wire tests pin the shapes. See `sprint-runs/encode-contract-audit-2026-06-10.md`. |
