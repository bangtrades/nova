...
| Encode-side contract audit (`.convertToSnakeCase` vs camelCase zod) | discovered Jun 10 | The auth fix exposed the class: iOS's body encoder snake-cases keys that backend zod schemas declare camelCase — silently drops optional fields, 400s required ones. Audit every Endpoint body against its backend schema; the decode side has ContractTests, the encode side has nothing. |
