...
- **Do not use non-existent DynamicTypeSize members** — `.accessibilityExtraExtraExtraLarge` does NOT exist. The maximum size is `.accessibility5`. Valid accessibility sizes: `.accessibilityMedium`, `.accessibilityLarge`, `.accessibilityExtraLarge`, `.accessibilityExtraExtraLarge`, `.accessibility5`.
- **Do not use fixed sizes for custom drawn elements** — Dots, icons, spacers, and other non-text UI elements should use `@ScaledMetric` to scale with Dynamic Type. Always pair with `.frame(minWidth: 44, minHeight: 44)` for tap target compliance.
- **Do not forget `.accessibilityAction` on custom gestures** — `onLongPressGesture` with empty `perform:` (used for press tracking) is invisible to VoiceOver. Add `.accessibilityAction { onTap() }` and `.accessibilityHint("Double tap to...")` alongside `.accessibilityAddTraits(.isButton)`.
