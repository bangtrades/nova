...
| `Instance method 'lock' is unavailable from asynchronous contexts` | Is `NSLock` being used inside an `async` function? | Extract lock scope to a synchronous helper method; call the helper from async. For `@MainActor` classes, consider removing the lock entirely (already serialized). | `references/threading.md` |
| `Conformance of 'X' to protocol 'YDelegate' crosses into main actor-isolated code` | Is the class `@MainActor` but conforming to a non-isolated delegate protocol? | Make delegate methods `nonisolated` and hop to `@MainActor` via `Task { @MainActor in }` for state mutations. | `references/actors.md` |
| `Mutation of captured var 'X' in concurrently-executing code` | Is a local `var` being mutated inside a callback or closure that may run concurrently? | Replace callback-based pattern with `withCheckedContinuation` for one-shot values; use `Task { @MainActor in }` for repeated callbacks. | `references/threading.md` |
| `Main actor-isolated property 'X' can not be mutated from a nonisolated context` | Is a Timer/NotificationCenter callback mutating `@MainActor` state? | Timer and NotificationCenter callbacks are nonisolated. Wrap mutations in `Task { @MainActor in }`. | `references/actors.md` |
