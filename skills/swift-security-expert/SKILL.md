...
- [ ] **12. Debug logging never leaks secrets** — All `#if DEBUG` logging blocks sanitize HTTP headers before printing. Bearer tokens, API keys, and session cookies must be redacted from debug output. Pattern: filter `Authorization` header in `allHTTPHeaderFields` before `print()`. A leaked token in a console log can be captured by crash reporters or shared screenshots. → `common-anti-patterns.md`

- [ ] **13. OAuth client IDs come from environment, not source** — Client IDs and secrets loaded from `ProcessInfo.processInfo.environment` or `Bundle.main.infoDictionary` (via `.xcconfig`), never hardcoded as string literals. Placeholder strings like `"YOUR_CLIENT_ID"` must not ship. → `credential-storage-patterns.md`

- [ ] **14. Child data encrypted at rest (COPPA/kids apps)** — For apps targeting children: child PII (name, birthdate, interaction history) encrypted with AES-GCM via CryptoKit, key stored in Keychain with `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`. Parental consent tracked per data category with timestamped records. UserDefaults never used for child PII. → `cryptokit-symmetric.md`

- [ ] **15. Certificate pinning on API requests** — SPKI (Subject Public Key Info) pinning via `URLSessionDelegate` or `NSPinnedDomains` in Info.plist. Use SHA-256 hash of the public key, NOT leaf certificate (rotates annually). Bridge `SecTrustCopyCertificateChain` result to `[SecCertificate]` with `as?` — CFArray does NOT support Swift subscripts. → `certificate-trust.md`
